...
parser.add_argument("--dataset-format", type=str, required=True, choices=["kitti", "cityscapes","sintel"])
...
def dump_sintel_example(scene):
    
    scene_name = scene.split('/')[-1]
    dump_dir = args.dump_root/scene_name
    dump_dir.makedirs_p()
    frame_nb = 0
    
    for sample in data_loader.get_scene_imgs(scene):
        img = sample
        dump_img_file = dump_dir/'{}.jpg'.format(frame_nb)
        scipy.misc.imsave(dump_img_file, img)
        frame_nb = frame_nb + 1
...
def main():
    ...
    if args.dataset_format == 'sintel':
        from sintel_loader import sintel_loader
        data_loader = sintel_loader(args.dataset_dir,
                                        img_height=args.height,
                                        img_width=args.width)

    print('Retrieving frames')
    if args.dataset_format == 'sintel':
        Parallel(n_jobs=args.num_threads)(delayed(dump_sintel_example)(scene) for scene in tqdm(data_loader.scenes))
    else:
