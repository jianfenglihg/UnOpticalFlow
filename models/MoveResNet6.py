...
class MoveResNet6(nn.Module):

    def __init__(self, nb_ref_imgs=1, output_exp=True):
        super(MoveResNet6, self).__init__()
        ...
        if self.output_exp:
            ...
            self.pred_mask6 = nn.Conv2d(upconv_planes[0], self.nb_ref_imgs*3, kernel_size=3, padding=1)
            self.pred_mask5 = nn.Conv2d(upconv_planes[1], self.nb_ref_imgs*3, kernel_size=3, padding=1)
            self.pred_mask4 = nn.Conv2d(upconv_planes[2], self.nb_ref_imgs*3, kernel_size=3, padding=1)
            self.pred_mask3 = nn.Conv2d(upconv_planes[3], self.nb_ref_imgs*3, kernel_size=3, padding=1)
            self.pred_mask2 = nn.Conv2d(upconv_planes[4], self.nb_ref_imgs*3, kernel_size=3, padding=1)
            self.pred_mask1 = nn.Conv2d(upconv_planes[5], self.nb_ref_imgs*3, kernel_size=3, padding=1)

    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                nn.init.xavier_uniform(m.weight.data) ### edit here
    ...
    def forward(self, target_image, ref_imgs):
        ...
        if self.output_exp:
            ...
            exp_mask6 = self.pred_mask6(out_upconv6)
            exp_mask5 = self.pred_mask5(out_upconv5)
            exp_mask4 = self.pred_mask4(out_upconv4)
            exp_mask3 = self.pred_mask3(out_upconv3)
            exp_mask2 = self.pred_mask2(out_upconv2)
            exp_mask1 = 8*self.pred_mask1(out_upconv1)
